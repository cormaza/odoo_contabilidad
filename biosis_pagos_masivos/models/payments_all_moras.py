...
class account_register_payments(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        amount = 0
        sw = 0
        #Primero valido si una de las facturas es MORA, tengo que RESTRINGIR que solo sean MORAS
        for factura in invoices:
            id = factura.id
            fact_mora = self.env['account.invoice'].search([('id', '=', id)], limit=1)
            if fact_mora.is_boleta_mora == True:
                amount = amount + fact_mora.amount_total
                pass
            else:
                sw = 1
        if sw == 1:
