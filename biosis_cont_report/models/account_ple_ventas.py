...
class PleVentas(models.Model):
    ...
    @api.multi
    def get_ple_ventas(self, company_id, fecha_reporte, fecha_inicio, fecha_fin):
        ple_ventas_res = ''
        ...
        if len(invoices_nuevos) == 0 and len(invoices_old) == 0:
            ...
            if len(invoices_nuevos) > 0:
                ple_nuevos = self.create_ple_items_ventas(company_id, invoices_nuevos, fecha_reporte, fecha_inicio, fecha_fin)
                ple_ventas_res = ple_ventas_res + ple_nuevos

            if len(invoices_old) > 0:
                ple_modificados = self.update_ple_items_ventas(company_id, invoices_old, fecha_inicio, fecha_fin)
                ple_ventas_res = ple_ventas_res + ple_modificados

            return ple_ventas_res

    @api.multi
    def create_ple_items_ventas(self, company_id, invoices, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
    ...
    @api.multi
    def update_ple_items_ventas(self, company_id, invoices, fecha_inicio, fecha_fin):
        ple_items = ''
        for invoice in invoices:
