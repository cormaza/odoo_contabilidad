...
class PleCompras(models.Model):
    ...
    @api.multi
    def get_ple_compras(self, company_id, fecha_reporte, fecha_inicio, fecha_fin):
        ple_compras_res = ''
        # ED sin ple
        ...
        if len(invoices_nuevos) == 0 and len(invoices_old) == 0:
            ...
            if len(invoices_nuevos) > 0:
                ple_nuevos = self.create_ple_items_compras(company_id, invoices_nuevos, fecha_reporte, fecha_inicio, fecha_fin)
                ple_compras_res = ple_compras_res + ple_nuevos

            if len(invoices_old) > 0:
                ple_modificados = self.update_ple_items_compras(company_id, invoices_old, fecha_fin)
                ple_compras_res = ple_compras_res + ple_modificados

        return ple_compras_res

    @api.multi
    def create_ple_items_compras(self, company_id, invoices, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
    ...
    @api.multi
    def update_ple_items_compras(self, company_id, invoices, fecha_fin):
        ple_items = ''
        for invoice in invoices:
