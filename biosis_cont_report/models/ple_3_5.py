...
class Ple_3_5(models.Model):
    _name = 'account.ple.3.5'
    _description = u'PLE - DETALLE DEL SALDO DE LA CUENTA 16 CUENTAS POR COBRAR DIVERSAS - TERCEROS O CUENTA 17'
    ...
    tipo_doc_ter_4 = fields.Char(string=u'Tipo documento Emisor', default=u'')
    numero_doc_ter_5 = fields.Char(string=u'Número documento Emisor', default=u'')
    razon_social_ter_6 = fields.Char(string=u'Razón Social/Nombres', default=u'')
    ...
    def get_ple_line(self):
        return self.periodo_1 + '|' + self.cuo_2 + '|' + self.move_cuo_3 + '|' + self.tipo_doc_ter_4 + '|' + self.numero_doc_ter_5 + '|' \
               + self.razon_social_ter_6 + '|' + self.fecha_e_7 + '|' + self.mont_cobrar_8 + '|' + self.estado_9 + '|' + '\n'
    ...
    def get_ple(self, company_id, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        ple_list = self.env['account.ple.3.5'].search([
    ...
    def create_ple_items(self, company_id, ple_new, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
        ple_model = self.env['account.ple.3.5']
        ...
        for line in ple_new:
            ...
            ple_item_vals = {
                ...
                'razon_social_ter_6': line.partner_id.registration_name if line.partner_id.registration_name
    ...
    def update_ple_items(self, company_id, ple_update, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        for line in ple_update:
            ple_actual = self.env['account.ple.3.5'].search([
                ('move_line_id', '=', line.id)
            ])
            if ple_actual.create_date < line.write_date:
                ...
                if ple_actual.fecha_e_7 != datetime.datetime.strptime(line.date, '%Y-%m-%d').strftime(
                        '%d/%m/%Y'):
                    flag_change = True
                if ple_actual.tipo_doc_ter_4 != line.partner_id.catalog_06_id.code:
                    flag_change = True
                if ple_actual.numero_doc_ter_5 != line.partner_id.vat:
                    flag_change = True
