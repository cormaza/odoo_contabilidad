...
class Ple_3_4(models.Model):
    _name = 'account.ple.3.4'
    _description = u'PLE - DETALLE DEL SALDO DE LA CUENTA 14 CUENTAS POR COBRAR AL PERSONAL, A LOS ACCIONISTAS'
    ...
    tipo_doc_acc_4 = fields.Char(string=u'Tipo documento Emisor', default=u'')
    numero_doc_acc_5 = fields.Char(string=u'Número documento Emisor', default=u'')
    razon_social_acc_6 = fields.Char(string=u'Razón Social/Nombres', default=u'')
    ...
    def get_ple_line(self):
        return self.periodo_1 + '|' + self.cuo_2 + '|' + self.move_cuo_3 + '|' + self.tipo_doc_acc_4 + '|' + self.numero_doc_acc_5 + '|' \
               + self.razon_social_acc_6 + '|' + self.fecha_e_7 + '|' + self.mont_cobrar_8 + '|' +self.estado_9 + '|' + '\n'
    ...
    def get_ple(self, company_id, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        ple_list = self.env['account.ple.3.4'].search([
    ...
    def create_ple_items(self, company_id, ple_new, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
        ple_model = self.env['account.ple.3.4']
        ...
        for line in ple_new:
            ...
            ple_item_vals = {
                ...
                'tipo_doc_acc_4': line.partner_id.catalog_06_id.code if line.partner_id else '-',
                'numero_doc_acc_5': line.partner_id.vat if line.partner_id else '-',
                'razon_social_acc_6': line.partner_id.registration_name if line.partner_id.registration_name
                else (line.partner_id.name if line.partner_id else '-'),  # SE APLICARA CUANDO ESTE COMPLETADO CENTRO DE COSTOS
                'fecha_e_7': datetime.datetime.strptime(line.date, '%Y-%m-%d').strftime('%d/%m/%Y'),
                'mont_cobrar_8': str(line.balance) if str(line.balance) else '0.00',
    ...
    def update_ple_items(self, company_id, ple_update, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        for line in ple_update:
            ple_actual = self.env['account.ple.3.4'].search([
                ('move_line_id', '=', line.id)
            ])
            if ple_actual.create_date < line.write_date:
                ...
                if ple_actual.fecha_e_7 != datetime.datetime.strptime(line.date, '%Y-%m-%d').strftime(
                        '%d/%m/%Y'):
                    flag_change = True
                if ple_actual.tipo_doc_acc_4 != line.partner_id.catalog_06_id.code:
                    flag_change = True
                if ple_actual.numero_doc_acc_5 != line.partner_id.vat:
                    flag_change = True
