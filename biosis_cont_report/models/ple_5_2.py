...
class Ple_5_2(models.Model):
    _name = 'account.ple.5.2'
    _description = 'PLE para LIBRO DIARIO DE FORMATO SIMPLIFICADO'
    ...
    def get_ple(self, company_id, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        d_ple_list = self.env['account.ple.5.2'].search([
    ...
    def create_ple_items(self, company_id, move_line_new, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
        ple_diario = self.env['account.ple.5.2']
    ...
    def update_ple_items(self, company_id, move_line_update, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        for move_line in move_line_update:
            ple_actual = self.env['account.ple.5.2'].search([
                ('cuo_2', '=', move_line.move_id.cuo),
                ('move_cuo_3', '=', move_line.numero_asiento),
                ('company_id', '=', company_id.id)
            ], limit=1)
