...
class Ple_3_12(models.Model):
    _name = 'account.ple.3.12'
    _description = u'PLE - DETALLE DEL SALDO DE LA CUENTA 42 CUENTAS POR PAGAR COMERCIALES – TERCEROS Y LA CUENTA 43'
    ...
    tipo_doc_pro_4 = fields.Char(string=u'Tipo documento Proveedor', default=u'')
    numero_doc_pro_5 = fields.Char(string=u'Número documento Proveedor', default=u'')
    fecha_emision_6 = fields.Char(string=u'Fecha de emisión del Comprobante de Pago', required=True)
    nombre_pro_7 = fields.Char(string=u'Apellidos y nombres, Denominación o Razon social del proveedor', required=True)
    monto_cuenta_8 = fields.Char(string=u'Monto de cada cuenta por pagar al proveedor', default=u'-')
    ...
    def get_ple_line(self):
        return self.periodo_1 + '|' + self.cuo_2 + '|' + self.move_cuo_3 + '|' + self.tipo_doc_pro_4 + '|' + self.numero_doc_pro_5 + '|' \
               + self.fecha_emision_6 + '|' + self.nombre_pro_7 + '|' + self.monto_cuenta_8 + '|' + self.estado_9 + '|' + '\n'
    ...
    def get_ple(self, company_id, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        """
        proveedor_list = self.env['account.move.line'].search([
            ('date', '>=', fecha_inicio),
            ('date', '<=', fecha_fin),
            ('account_id.code', '=like', '43%'),
            ('move_id.state', '!=', 'draft'),
        ...
        """
        ple_list = self.env['account.ple.3.12'].search([
    ...
    def create_ple_items(self, company_id, ple_new, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
        ple_model = self.env['account.ple.3.12']
        ...
        for line in ple_new:
            ...
            ple_item_vals = {
                ...
                'move_cuo_3': 'M' + str(i),
                'tipo_doc_pro_4': line.partner_id.catalog_06_id.code if line.partner_id else '-',
                'numero_doc_pro_5': line.partner_id.vat if line.partner_id else '-',
                'fecha_emision_6': datetime.datetime.strptime(line.date, '%Y-%m-%d').strftime('%d/%m/%Y'),
                'nombre_pro_7': line.partner_id.registration_name if line.partner_id.registration_name
                else (line.partner_id.name if line.partner_id else '-'),
                'monto_cuenta_8':  str(line.residual) if line.residual > 0 else '0.00',
    ...
    def update_ple_items(self, company_id, ple_update, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        for line in ple_update:
            ple_actual = self.env['account.ple.3.12'].search([
                ('invoice_id', '=', line.id),
                ('company_id','=',company_id.id)
            ])
