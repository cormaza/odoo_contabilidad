...
class Ple_5_1(models.Model):
    _name = 'account.ple.5.1'
    ...
    def get_ple_line(self):
        return self.periodo_1+'|'+self.cuo_2+'|'+self.move_cuo_3+'|'+self.cuenta_cont_4+'|'+self.cunio_uea_un_up_5+'|'\
               ...
               +self.tipo_cpbt_10+'|'+self.serie_cpbt_11+'|'+self.numero_cpbt_12+'|'\
               +datetime.datetime.strptime(self.fecha_c_13, '%Y-%m-%d').strftime('%d/%m/%Y')+'|'\
               +datetime.datetime.strptime(self.fecha_v_14, '%Y-%m-%d').strftime('%d/%m/%Y')+'|'\
               +datetime.datetime.strptime(self.fecha_e_15, '%Y-%m-%d').strftime('%d/%m/%Y')+'|'\
               +self.glosa_16+'|'+self.glosa_referencial_17+'|'\
    ...
    def get_ple(self, company_id, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        ]).sorted(key=lambda r: int(r.move_id.cuo)).sorted(key=lambda r: int(r.id))

        d_ple_list = self.env['account.ple.5.1'].search([
            ('periodo_1', '=', fecha_reporte),
            ('company_id', '=', company_id.id)
        ])

        move_lines_d = [line.move_line_id for line in d_ple_list]
    ...
    def create_ple_items(self, company_id, move_line_new, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
        ple_diario = self.env['account.ple.5.1']

        for move_line in move_line_new:
            if datetime.datetime.strptime(move_line.move_id.date, '%Y-%m-%d').date() >= fecha_inicio and datetime.datetime.strptime(
                    move_line.move_id.date, '%Y-%m-%d').date() <= fecha_fin:
                ple_item_estado_21 = u'1'
            elif datetime.datetime.strptime(move_line.move_id.date, '%Y-%m-%d').date() <= fecha_inicio:
    ...
    def update_ple_items(self, company_id, move_line_update, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
        for move_line in move_line_update:
            ple_actual = self.env['account.ple.5.1'].search([
                ('cuo_2', '=', move_line.move_id.cuo),
                ('move_cuo_3', '=', move_line.numero_asiento),
                ('company_id', '=', company_id.id)
            ], limit=1)
            if ple_actual.write_date < move_line.write_date:
                ...
                ple_item_vals = {
                    ...
                    'numero_doc_pro_9': move_line.invoice_id.partner_id.vat if move_line.invoice_id.partner_id and move_line.invoice_id.type == 'in_invoice' else '',
                    'tipo_cpbt_10': move_line.invoice_id.tipo_comprobante_id.code if move_line.invoice_id.tipo_comprobante_id.code else '00',
                    'serie_cpbt_11': move_line.invoice_id.numero_comprobante.split('-')[
                        0] if move_line.invoice_id.numero_comprobante else '-',
                    'numero_cpbt_12': move_line.invoice_id.numero_comprobante.split('-')[
                        1] if move_line.invoice_id.numero_comprobante else '-',
                    #'fecha_c_13': datetime.datetime.strptime(move_line.date_maturity, '%Y-%m-%d').strftime('%d/%m/%Y'),
                    #'fecha_v_14': datetime.datetime.strptime(move_line.date, '%Y-%m-%d').strftime('%d/%m/%Y'),
                    #'fecha_e_15': '01/01/0001',
                    'fecha_c_13': move_line.date_maturity,
